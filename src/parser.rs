...
macro_rules! e_string {
  ($text: expr, $p: ident) => {
    return Err(format!("(Ln {}, Col {}) {}", $p.curtok.ln, $p.curtok.col, $text))
  };
}
...
impl Parser {
  ...
  pub fn parse(&mut self) -> Result<Program, String> {
    ...
    while !self.curtok_is(EOF) {
      match self.curtok.kind {
        ...
        _=> {
          program.push(Statement::Expression(self.parse_expression(Precedence::Iota)?));
        }
      }
    }
  ...
  fn error(&self, text: &str) -> Result<Token, String> {
    Err(format!("(Ln {}, Col {}) {}", self.curtok.ln, self.curtok.col, text))
  }
  ...
  fn expect_peek(&mut self, kind: TokenKind) -> Result<Token, String> {
    ...
    let msg = format!("Expected the next token to be {:?}, instead got {:?}", kind, peek.kind);
    e_string!(msg, self)
  }
  ...
  fn expect_kind(&mut self, kind: TokenKind) -> Result<Token, String> {
    if self.curtok.kind == kind {
      ...
      e_string!(format!("Expected {:?}. Instead got {:?}", kind, self.curtok.kind), self)
    }
  }
  ...
  fn parse_while(&mut self) -> Result<Statement, String> {
    ...
    return Ok(Statement::While{condition: condition, body: body});
  }
  ...
  fn parse_var(&mut self) -> Result<Statement, String> {
    ...
    let res = Statement::VarDeclaration{name: iden.text, value: expr};
  ...
  fn parse_statement(&mut self) -> Result<Statement, String> {
    match self.curtok.kind {
      ...
      _ => Ok(Statement::Expression(self.parse_expression(Precedence::Iota)?)),
    }
  }
  ...
  */ 
  fn parse_expression(&mut self, precedence: Precedence) -> Result<Expression, String> {
    let mut left = match self.curtok.kind {
      IntegerLiteral => {
        ...
        Expression::Int(clone.text.parse().unwrap())
      },
      Identifier => {
        Expression::Identifier(self.curtok.text.clone())
      },
      Lparen => {
        self.next();
      ...
      StringLiteral => {
        Expression::Str(self.curtok.text.clone())
      },
      TrueLiteral => {
    ...
    if self.curtok_is(TokenKind::Semicolon) { self.next(); }
  ...
  fn parse_infix_expression(&mut self, left: &Expression) -> Result<Option<Expression>, String> {
    match self.curtok.kind {
      ...
      | GreaterEquals | LessEquals => {
        ...
        return Ok(Some(Expression::BinOp(Box::new(left.clone()), token.kind, Box::new(right))));
      }
      Assign => {
        self.next();
        ...
        Ok(Some(Expression::Assign(Box::new(left.clone()), Box::new(right))))
      },
      _=> Ok(None)
    }
  }
  ...
  fn parse_postfix_expression(&mut self, left: &Expression) -> Result<Option<Expression>, String> {
    match self.curtok.kind {
      Lparen => {
        ...
        if let Expression::Identifier(v) = left {
          let res = Expression::Call{name: v.clone(), args: args};
